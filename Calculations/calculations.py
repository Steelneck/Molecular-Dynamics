...
def Self_diffuse(MSD, timeStepIndex, interval, time_step):
    ...
    Args:
        MSD (float): Mean square displacement returned by `MSD_Calc`
        timeStepIndex (int)
    """
    try:
...
def calc_instantaneous_pressure(myAtoms, trajObject, superCellSize, timeStepIndex):
    ...
    Args:
        myAtoms (Lattice)
        trajObject (TrajectoryReader)
        superCellSize (int): The repetition of unit cell in each direction in 3D space.
        timeStepIndex (int)"""
    try: 
...
def calc_internal_pressure(myAtoms, trajObject, eq_index, superCellSize):
    ...
    Note:
        trajObject must contain the atoms objects when the system has reached equilibrium,
    ...
    Args:
        myAtoms (Lattice)
        trajObject (TrajectoryReader)
        eq_index (int): The index for the current equilibrium state

    Returns:
        avgTemp (float): Sampled internal temperature in units of Kelvin.
    """
...
def cohesive_energy(myAtoms, trajObject, eq_index):
    """
    Note:
        Requires usage of the EAM calculator for reasonable output!
    ...
    Returns:
        avgEcoh (float): Sampled potential energy per atom (i.e. cohesive energy of material) in units of electronvolt per atom.
    """
    try:
        eq_length = len(trajObject) - eq_index
        eqEcoh = 0
...
def debye_temperature(trajObject, MSD, eq_index):
    ...
    Args:
        trajObject (TrajectoryReader)
        MSD (float): Mean square displacement as returned by `MSD_calc`.

    Returns:
        avgDebye (float): Sampled Debye temperature in units of Kelvin.
    """
    try: 
        print(units._hbar)
        eq_length = len(trajObject) - eq_index
...
""" Calculates the lattice constants.
    ...
    Args:
        atomName (str): Name of atom to use in file name
        atomsCalculator (str): Calculator to be used
...
def calc_bulk_modulus(atoms):
 """
    ...
    Based on the volume, this can also be used for calculation of lattice constant for cubic crystal structures.
    """
    try:
