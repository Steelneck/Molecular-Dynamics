...
def Heat_Capcity_NVE(avg_k, avg_k_squared, avg_temp, bulk_mass, Nr_of_atoms):   
    try:

        # Mass of the crystal 

        heat_capcity = ((1.5*Nr_of_atoms*units.kB)*((1-((avg_k_squared-(avg_k**2))/(1.5*((units.kB**2)*(avg_temp**2))))**(-1))))*1.602*10**(-19)
